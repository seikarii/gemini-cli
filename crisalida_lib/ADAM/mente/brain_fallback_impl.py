...
class BrainFallback:
    ...
    def __init__(
        ...
        self.chaotic_core: ChaoticCognitiveCore | None = None
        if getattr(self.config, "ENABLE_CHAOTIC_CORE", False):
            try:
                self.chaotic_core = ChaoticCognitiveCore(
                    n_dimensions=int(getattr(self.config, "CHAOTIC_CORE_DIMENSIONS", 100))
                )
                logger.info("ChaoticCognitiveCore initialized.")
            except Exception:
                logger.exception("Failed to initialize ChaoticCognitiveCore")
    ...
    async def step(self, context: dict[str, Any]) -> dict[str, Any]:
        with self._lock:
            ...
            try:
                recalled_experiences = self.recall_fn(context)
                if recalled_experiences:
                    context["recalled_experiences"] = recalled_experiences
            except Exception:
                logger.exception("Failed to recall experiences from memory")
            ...
            if self.chaotic_core:
                try:
                    chaotic_result = self.chaotic_core.think(problem_spec=context)
                    if chaotic_result and chaotic_result.get("solution"):
                        self._create_film_from_chaotic_solution(chaotic_result["solution"], context)
                except Exception:
                    logger.exception("ChaoticCognitiveCore.think() failed")
    ...
    def _select_film(self, context: dict[str, Any]) -> str | None:
        with self._lock:
            ...
            recalled_experiences = context.get("recalled_experiences", [])
            ...
            for film_id, film in self.films.items():
                relevance_score = 0.0
                if recalled_experiences and hasattr(recalled_experiences, '__iter__'):
                    # Calculate relevance based on shared tags
                    film_tags = set(film.tags)
                    for exp in recalled_experiences:
                        if hasattr(exp, 'get'):
                            exp_tags = set(exp.get("tags", []))
                            common_tags = film_tags.intersection(exp_tags)
                            if common_tags:
                                relevance_score += len(common_tags)

                # Combine fitness and relevance
                final_score = film.fitness + relevance_score
                
                if final_score > best_score:
                    best_score = final_score
                    best_film_id = film_id

            if best_film_id:
                film = self.films[best_film_id]
                entry = (
                    film.entry
                    if film.entry
                    else (next(iter(film.nodes)) if film.nodes else "")
                )
                self.current = (best_film_id, entry)
                return best_film_id
    ...
    def _create_film_from_chaotic_solution(self, solution: dict[str, Any], context: dict[str, Any]) -> None:
        """Creates a new film from a solution generated by the ChaoticCognitiveCore."""
        try:
            confidence = solution.get("confidence", 0.5)
            if confidence < getattr(self.config, "CHAOTIC_CORE_CONFIDENCE_THRESHOLD", 0.7):
                return

            film_id = f"chaotic_film_{gen_id()}"
            action_name = f"chaotic_{solution.get("type", "emergent")}"
            node_id = f"{film_id}_n0"

            node = FilmNode(
                id=node_id,
                action=action_name,
                params=solution.get("details", {}),
            ...
            film = Film(
                ...
                tags=["chaotic", f"confidence_{confidence:.2f}"],
            )

            self.films[film_id] = film
            logger.info(f"Created new film from chaotic solution: {film_id}")
        except Exception:
            logger.exception("Failed to create film from chaotic solution")
