  ...
  describe('End-to-End RAG Functionality', () => {
    it('should integrate ChatRecordingService with PromptContextManager', async () => {
      ...
      const conversationHistory: Content[] = [
        {
          role: 'user',
          parts: [{ text: 'Hello, I need help with TypeScript interfaces.' }],
        },
        {
          role: 'model',
          parts: [
            {
              text: "I'd be happy to help with TypeScript interfaces! What specific aspect would you like to know about?",
            },
          ],
        },
        {
          role: 'user',
          parts: [
            { text: 'How do I create optional properties in an interface?' },
          ],
        },
        {
          role: 'model',
          parts: [
            {
              text: 'You can create optional properties by adding a question mark (?) after the property name. For example: interface User { name: string; age?: number; }',
            },
          ],
        },
        {
          ...
          parts: [
            {
              text: 'Can you show me more complex examples with nested interfaces?',
            },
          ],
        },
        {
          ...
          parts: [
            {
              text: 'Certainly! Here are some complex interface examples with nesting and optional properties...',
            },
          ],
        },
      ];
      ...
      const optimizedResult =
        await chatRecordingService.getOptimizedHistoryForPrompt(
          conversationHistory,
          2000, // Token budget
      ...
      // Verify metadata
      expect(optimizedResult.metaInfo.originalMessageCount).toBe(
        conversationHistory.length,
      );
      expect(optimizedResult.metaInfo.finalMessageCount).toBeGreaterThan(0);
      expect(optimizedResult.metaInfo.finalMessageCount).toBeLessThanOrEqual(
        conversationHistory.length,
      );
      expect(typeof optimizedResult.metaInfo.compressionApplied).toBe(
        'boolean',
      );

      console.log(
        '✅ ChatRecordingService processes external history correctly',
      );
    });
    ...
    it('should handle token budget constraints with compression', async () => {
      ...
      const longConversation: Content[] = Array.from(
        { length: 50 },
        (_, i) => ({
      ...
      expect(result.metaInfo.finalMessageCount).toBeLessThanOrEqual(
        longConversation.length,
      );

      // Test with more generous budget
      const generousResult =
        await chatRecordingService.getOptimizedHistoryForPrompt(
          longConversation,
      ...
      expect(generousResult.metaInfo.finalMessageCount).toBeGreaterThanOrEqual(
        result.metaInfo.finalMessageCount,
      );

      console.log(
        `Restricted result: ${result.estimatedTokens} tokens, ${result.metaInfo.finalMessageCount} messages`,
      );
      console.log(
        `Generous result: ${generousResult.estimatedTokens} tokens, ${generousResult.metaInfo.finalMessageCount} messages`,
      );
    ...
    it('should maintain conversation context integrity', async () => {
      ...
      const contextualConversation: Content[] = [
        {
          role: 'user',
          parts: [
            { text: "I'm working on a React component called UserProfile." },
          ],
        },
        {
          role: 'model',
          parts: [
            {
              text: 'Great! I can help you with the UserProfile component. What specific functionality are you implementing?',
            },
          ],
        },
        {
          ...
          parts: [
            {
              text: 'I need to display user avatar, name, and bio. The avatar should be clickable.',
            },
          ],
        },
        {
          role: 'model',
          parts: [{ text: "Here's a UserProfile component structure..." }],
        },
        {
          role: 'user',
          parts: [{ text: 'How do I handle the avatar click event?' }],
        },
      ];
      ...
      expect(result.metaInfo.finalMessageCount).toBe(
        contextualConversation.length,
      );
      ...
      expect(contents[contents.length - 1].parts[0].text).toContain(
        'avatar click',
      );
    ...
    it('should return correct compression statistics', async () => {
      ...
      const mediumConversation: Content[] = Array.from(
        { length: 20 },
        (_, i) => ({
          role: i % 2 === 0 ? 'user' : 'model',
          parts: [
      ...
      if (result.metaInfo.compressionApplied) {
        ...
        expect(stats.compressedMessages).toBe(
          result.metaInfo.finalMessageCount,
        );
