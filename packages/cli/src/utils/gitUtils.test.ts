...
describe('isGitHubRepository', async () => {
  ...
  it('returns false if the git command fails', async () => {
    vi.spyOn(child_process, 'exec').mockImplementation((...args: unknown[]) => {
      ...
      if (typeof cb === 'function') (cb as (err: unknown, stdout?: string) => void)(new Error('oops'));
  ...
  it('returns false if the remote is not github.com', async () => {
    vi.spyOn(child_process, 'exec').mockImplementationOnce((...args: unknown[]) => {
      const cb = args[args.length - 1] as unknown;
      if (typeof cb === 'function') (cb as (err: unknown, stdout?: string) => void)(null, 'https://gitlab.com');
  return undefined as unknown as child_process.ChildProcess;
    });
  await expect(isGitHubRepository()).resolves.toBe(false);
  });
...
describe('getGitHubRepoInfo', async () => {
  ...
  it('throws an error if owner/repo could not be determined', async () => {
    vi.spyOn(child_process, 'exec').mockImplementationOnce((...args: unknown[]) => {
      const cb = args[args.length - 1] as unknown;
      if (typeof cb === 'function') (cb as (err: unknown, stdout?: string) => void)(null, '');
  return undefined as unknown as child_process.ChildProcess;
    });
  await expect(getGitHubRepoInfo()).rejects.toThrowError(/Owner & repo could not be extracted from remote URL/);
  });

  it('returns the owner and repo', async () => {
    vi.spyOn(child_process, 'exec').mockImplementationOnce((...args: unknown[]) => {
      const cb = args[args.length - 1] as unknown;
      if (typeof cb === 'function') (cb as (err: unknown, stdout?: string) => void)(null, 'https://github.com/owner/repo.git ');
  return undefined as unknown as child_process.ChildProcess;
    });
  await expect(getGitHubRepoInfo()).resolves.toEqual({ owner: 'owner', repo: 'repo' });
  });
});
...
describe('getGitRepoRoot', async () => {
  ...
  await expect(getGitRepoRoot()).rejects.toThrowError(/Git repo returned empty value/);
  });

  it('returns the root', async () => {
    vi.spyOn(child_process, 'exec').mockImplementationOnce((...args: unknown[]) => {
      const cb = args[args.length - 1] as unknown;
      if (typeof cb === 'function') (cb as (err: unknown, stdout?: string) => void)(null, '/path/to/git/repo');
  return undefined as unknown as child_process.ChildProcess;
    });
