...
export class EmotionalEvaluator {
  ...
  constructor(affect_fn: ((ctx: Record<string, any>) => [number, number, number]) | null = null) {
  ...
  private _calculate_somatic_modifier(physiological_state: string): Record<string, number> {
    ...
    return mapping[physiological_state] || {"valence": 0.0, "dopamine": 0.0, "arousal": 0.0};
  }
}
export class ExecutivePFC {
  ...
  allow_action(
    ...
    const gate = long_term_gain - (
      base_inhibition * Math.max(0.0, 0.6 - urge_score)
    );
    return gate >= 0.0;
  }
}
export class BasalGanglia {
  ...
  pick(candidates: string[]): string {
    ...
    const weights = candidates.map(c => Math.exp(this.habit_scores[c] || 0.0));
...
export class PatternRecognizer {
  ...
  constructor(get_embedding: (x: any) => number[], config: AdamConfig = DEFAULT_ADAM_CONFIG) {
...
export class SimpleActuator {
  ...
  async execute_action(
    ...
    try {
      ...
      const tool_result: ToolCallResult = await (this.tool_registry as any).execute_tool(
        action, params
      ); // Pass params as a single object
...
export class BrainFallback {
  ...
  constructor(
    ...
    emit_event: ((eventType: string, data: Record<string, any>) => void) | null = null,
    ...
    this.recall_fn = recall_fn || ((x: any) => {
      const dim = _safe_float((this.config as any).EMBEDDING_DIM, 16);
      return [new Array(dim).fill(0.0), []];
    });

    this.ingest_fn = ingest_fn || ((...args: any[]) => {
      console.debug("ingest noop called with", args);
      return null;
    });

    this.emit_event = emit_event || ((eventType: string, data: Record<string, any>) => {
    ...
    this.patterns = new PatternRecognizer(get_embedding || ((x: any) => new Array(_safe_float((this.config as any).EMBEDDING_DIM, 16)).fill(0.0)), this.config);
    ...
    // Buffered EVA writes
    this._eva_buffer = [];
    this._eva_flush_interval = _safe_float((this.config as any).BRAIN_EVA_FLUSH_S, 30.0);
  ...
  async step(context: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (this.chaotic_core) {
      try {
        const chaotic_result = await (this.chaotic_core as any).think({ problem_spec: context });
    ...
    const crisis_threat_threshold = _safe_float((this.config as any).CRISIS_THREAT_THRESHOLD, 0.9);
  ...
  async _advance_film(film_id: string, ctx: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (node === undefined) {
      // fallback gracefully
      node_id = f.entry || (f.nodes && Object.keys(f.nodes).length > 0 ? Object.keys(f.nodes)[0] : "");
    ...
    const next_nodes = (f.edges || []).filter(e => e.src === node_id && e.condition(ctx)).map(e => e.dst);
  ...
  learn_from_outcome(
    ...
    if (impact_score > 0.8) { // High impact threshold
      ...
      const fitness_boost = (reward > 0) ? impact_score * 2.0 : -impact_score * 1.5;
  ...
  get_learning_statistics(): Record<string, any> {
    ...
    const total_fitness = allFilms.reduce((sum, f) => sum + (f.fitness || 0.0), 0);
    const average_fitness = total_fitness / total_films;

    const one_shot_films = allFilms.filter(f => f.tags && f.tags.includes("one_shot")).length;
    const complex_films = allFilms.filter(f => f.tags && f.tags.includes("complex")).length;
