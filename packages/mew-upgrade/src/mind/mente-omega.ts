...
class EntityMemory {
  ...
  private calculateRelevanceScore(query: string, node: MemoryNode): number {
    ...
    const recencyBoost =
      Math.max(
        0,
        1 - (Date.now() - node.timestamp) / (1000 * 60 * 60 * 24 * 7),
      ) * 0.1;
    const accessBoost = Math.min(0.2, node.accessCount * 0.02);

    return Math.min(
      1,
      textScore + importanceBoost + recencyBoost + accessBoost,
    );
  }
  ...
  exportState(): object {
    return {
      ...
      associationGraph: Array.from(this.associationGraph.entries()).map(
        ([k, v]) => [k, Array.from(v)],
      ),
      tagIndex: Array.from(this.tagIndex.entries()).map(([k, v]) => [
        k,
        Array.from(v),
      ]),
      temporalIndex: Array.from(this.temporalIndex.entries()).map(([k, v]) => [
        k,
        Array.from(v),
      ]),
  ...
  importState(state: any): void {
    ...
    if (state.associationGraph && Array.isArray(state.associationGraph)) {
      this.associationGraph = new Map(
        state.associationGraph.map(([k, v]: [string, string[]]) => [
          k,
          new Set(v),
        ]),
      );
    ...
    if (state.temporalIndex && Array.isArray(state.temporalIndex)) {
      this.temporalIndex = new Map(
        state.temporalIndex.map(([k, v]: [number, string[]]) => [
          k,
          new Set(v),
        ]),
      );
...
export class MenteOmega implements Persistable {
  ...
  async process(
    ...
    const contextForAlfa = this.prepareContextForAlfa(
      userRequest,
      enhancedMemories,
      insights,
      processingContext,
    ...
    let response =
      'I understand your request and am processing it thoughtfully.';
        ...
        const result = await (this.menteAlfa as any).generateContent?.(
          contextForAlfa,
          {
            temperature: Math.max(
              0.1,
              Math.min(0.9, this.cognitiveState.curiosity),
            ),
  ...
  private updateCognitiveState(analysis: any): void {
    ...
    this.cognitiveState.emotionalState.valence +=
      analysis.emotionalValence * 0.1;
    this.cognitiveState.emotionalState.arousal += analysis.urgency * 0.2;

    // Clamp values
    this.cognitiveState.emotionalState.valence = Math.max(
      -1,
      Math.min(1, this.cognitiveState.emotionalState.valence),
    );
    this.cognitiveState.emotionalState.arousal = Math.max(
      0,
      Math.min(1, this.cognitiveState.emotionalState.arousal),
    );
    ...
    this.cognitiveState.curiosity = Math.max(
      0.1,
      Math.min(1, this.cognitiveState.curiosity),
    );
  }
  ...
  private generateInsights(
    ...
    const coherence =
      memories.length > 0
        ? memories.reduce(
            (sum, mem) => sum + mem.associatedNodes.length * 0.1,
            0,
          ) / memories.length
        : 0;
    ...
    const novelty = Math.max(
      0,
      1 -
        this.processingHistory.filter(
          (h) => this.calculateSimilarity(h.userRequest, request) > 0.7,
        ).length /
          10,
    );
    ...
    const complexity = Math.min(
      1,
      request.split(/\s+/).length / 50 + memories.length / 20,
    );
    ...
    const emotionalResonance =
      memories.length > 0
        ? memories.reduce(
            (sum, mem) => sum + Math.abs(mem.valence) + mem.arousal,
            0,
          ) /
          (memories.length * 2)
        : 0.5;
...
- Rapport level: ${processingContext.emotionalContext.rapport}`,
  ...
  private postProcessCognitiveUpdate(
    ...
    if (
      insights.complexity > 0.7 &&
      !this.cognitiveState.currentGoals.includes('deep_analysis')
    ) {
    ...
    if (insights.confidence > 0.8) {
      ...
      this.cognitiveState.attention = Math.min(
        1,
        this.cognitiveState.attention * 1.1,
      ); // Increase focus
    }

    // Update focus areas
    this.cognitiveState.focus = response
      .toLowerCase()
      .split(/\s+/)
...
Focus areas: ${this.cognitiveState.focus.join(', ') || 'none'}.`;
  ...
  async processIncomingData(
    ...
    const importance = this.calculateDataImportance(
      incomingData,
      currentMission,
    );
    ...
    await this.memory.ingest(
      ...
      [
        'external',
        'processed',
        currentMission.toLowerCase().replace(/\s+/g, '_'),
      ],
      [],
    );
  ...
  private extractEmotionalContext(data: unknown): {
    valence: number;
    arousal: number;
  } {
    const dataStr = String(data).toLowerCase();
    ...
    const positiveCount = positiveWords.filter((word) =>
      dataStr.includes(word),
    ).length;
    const negativeCount = negativeWords.filter((word) =>
      dataStr.includes(word),
    ).length;
    const arousalCount = highArousalWords.filter((word) =>
      dataStr.includes(word),
    ).length;
  ...
  getMemoryStats(): object {
    ...
    return {
      ...
      averageImportance:
        allNodes.length > 0 ? totalImportance / allNodes.length : 0,
      ...
      oldestMemory:
        allNodes.length > 0
          ? new Date(Math.min(...allNodes.map((n) => n.timestamp)))
          : null,
      newestMemory:
        allNodes.length > 0
          ? new Date(Math.max(...allNodes.map((n) => n.timestamp)))
          : null,
    };
  }
  ...
  importState(state: unknown): void {
    ...
    if (stateObj.cognitiveState) {
      this.cognitiveState = {
        ...this.cognitiveState,
        ...stateObj.cognitiveState,
      };
    }

    if (
      stateObj.processingHistory &&
      Array.isArray(stateObj.processingHistory)
    ) {
