...
class EnhancedXPathParser {
  parse(xpath: string) {
    ...
    const predicates = Array.from(expr.matchAll(/\[([^\]]+)\]/g)).map(m => m[1].trim());
    for (const pred of predicates) {
      ...
      if (attrEq) {
        parts.conditions.push({ type: 'attribute', field: attrEq[1], operator: 'eq', value: attrEq[3] });
        continue;
      }
      // attribute not equals
      const attrNe = pred.match(/^@([\w$]+)\s*!=\s*(['"])([\s\S]+)\2$/);
      if (attrNe) {
        parts.conditions.push({ type: 'attribute', field: attrNe[1], operator: 'ne', value: attrNe[3] });
        continue;
      }
      // contains(@attr, 'val')
      const contains = pred.match(/^contains\(\s*@([\w$]+)\s*,\s*(['"])([\s\S]+)\2\s*\)$/);
      if (contains) {
        parts.conditions.push({ type: 'attribute', field: contains[1], operator: 'contains', value: contains[3] });
        continue;
      }
      // starts-with(@attr, 'val')
      const starts = pred.match(/^starts-with\(\s*@([\w$]+)\s*,\s*(['"])([\s\S]+)\2\s*\)$/);
      if (starts) {
        parts.conditions.push({ type: 'attribute', field: starts[1], operator: 'starts_with', value: starts[3] });
        continue;
      }
 ...
 */
export function findNodes(sourceFile: SourceFile, query: DictionaryQuery | string | { xpath?: string; custom?: (n: Node) => boolean }): Node[] {
  ...
  if (typeof query === 'object' && (query as any).custom && typeof (query as any).custom === 'function') {
 ...
 */
function findByCustom(sourceFile: SourceFile, fn: (n: Node) => boolean): Node[] {
  const found: Node[] = [];
 ...
 */
function findByXPath(sourceFile: SourceFile, parsed: ReturnType<EnhancedXPathParser['parse']>): Node[] {
  const candidates: Node[] = [];
  const nodeTest = parsed.nodeTest;
  if (!nodeTest) return [];
  const descendants = parsed.axis === 'child' ? sourceFile.getChildren() : sourceFile.getDescendants();

  for (const node of descendants) {
    // match by kind name (e.g. FunctionDeclaration, ClassDeclaration, VariableStatement...)
    const kindName = (node as any).getKindName ? (node as any).getKindName() : SyntaxKind[node.getKind()];
  ...
  if (typeof parsed.position === 'number') {
    return candidates.length >= parsed.position ? [candidates[parsed.position - 1]] : [];
  }
...
function extractAttributeValue(node: Node, field: string): string | undefined {
  try {
    ...
    if (prop && typeof prop === 'object' && typeof prop.getText === 'function') return prop.getText();
...
function matchesQuery(node: Node, query: DictionaryQuery): boolean {
  ...
  // Check kind
  if (conditions.kind) {
    const kinds = Array.isArray(conditions.kind) ? conditions.kind : [conditions.kind];
  ...
  if (conditions.parent) {
    ...
    let matched = false;
    while (parent) {
      if (matchesQuery(parent, { type: 'dictionary', conditions: conditions.parent } as any)) {
        matched = true;
        break;
      }
  ...
  if (conditions.child) {
    ...
    for (const child of children) {
      if (matchesQuery(child, { type: 'dictionary', conditions: conditions.child } as any)) {
