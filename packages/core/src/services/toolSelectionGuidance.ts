...
export interface TaskContext {
  ...
  operationType: 'read' | 'modify' | 'create' | 'search' | 'analyze' | 'unknown';
  /** Specific modification type if applicable */
  modificationType?: 'function' | 'class' | 'variable' | 'config' | 'structure' | 'text';
...
export class ToolSelectionGuidance {
  ...
  private static analyzeTaskContext(
    ...
    const referencedFiles = this.extractFileReferences(userMessage, conversationHistory);
    const operationType = this.detectOperationType(userMessage);
    const modificationType = this.detectModificationType(userMessage);
    const { hasRecentFailures, lastFailedTool, recentErrors } = this.analyzeRecentFailures(conversationHistory);
   ...
   */
  private static extractFileReferences(userMessage: string, history: Content[]): string[] {
    ...
    const allText = userMessage + ' ' + 
      history.slice(-3).map(c => this.extractTextFromContent(c)).join(' ');
   ...
   */
  private static detectOperationType(userMessage: string): TaskContext['operationType'] {
    const msg = userMessage.toLowerCase();
    
    if (msg.includes('read') || msg.includes('show') || msg.includes('display') || msg.includes('view')) {
      return 'read';
    }
    if (msg.includes('create') || msg.includes('new') || msg.includes('add') || msg.includes('generate')) {
      return 'create';
    }
    if (msg.includes('update') || msg.includes('modify') || msg.includes('change') || msg.includes('edit') || 
        msg.includes('fix') || msg.includes('refactor') || msg.includes('rename')) {
      return 'modify';
    }
    if (msg.includes('search') || msg.includes('find') || msg.includes('look for') || msg.includes('locate')) {
      return 'search';
    }
    if (msg.includes('analyze') || msg.includes('review') || msg.includes('check') || msg.includes('examine')) {
      return 'analyze';
    }
   ...
   */
  private static detectModificationType(userMessage: string): TaskContext['modificationType'] {
    const msg = userMessage.toLowerCase();
    
    if (msg.includes('function') || msg.includes('method') || msg.includes('procedure')) {
      return 'function';
    }
    if (msg.includes('class') || msg.includes('interface') || msg.includes('type') || msg.includes('struct')) {
      return 'class';
    }
    if (msg.includes('variable') || msg.includes('constant') || msg.includes('property') || msg.includes('field')) {
      return 'variable';
    }
    if (msg.includes('config') || msg.includes('json') || msg.includes('yaml') || msg.includes('xml') || 
        msg.includes('setting') || msg.includes('package.json')) {
      return 'config';
    }
    if (msg.includes('architecture') || msg.includes('structure') || msg.includes('organize') || msg.includes('module')) {
      return 'structure';
    }
  ...
  private static analyzeRecentFailures(history: Content[]): {
    ...
    for (const content of recentMessages) {
      ...
      if (text.includes('Error:') || text.includes('Failed:') || text.includes('could not') || 
          text.includes('failed') || text.includes('multiple matches') || text.includes('not found')) {
        errors.push(text.substring(0, 200));
        
        // Extract failed tool name
        const toolMatch = text.match(/tool[:\s]+(\w+)|(\w+)\s+failed|Error.*?(\w+_\w+)|replace.*?failed/i);
        if (toolMatch) {
          lastFailedTool = toolMatch[1] || toolMatch[2] || toolMatch[3] || 'replace';
        }
      }

      // Look for specific tool failure patterns
      if (text.includes('replace') && (text.includes('multiple matches') || text.includes('not found') || 
  ...
  private static recommendTools(context: TaskContext): ToolRecommendation {
    ...
    if (context.hasRecentFailures && context.lastFailedTool === 'replace') {
      ...
      if (codeFiles.length > 0 || context.modificationType === 'function' || context.modificationType === 'class') {
        return {
          primaryTool: 'upsert_code_block',
          alternatives: ['ast_edit', 'replace'],
          reasoning: 'Previous replace operation failed, switching to AST-based approach for code files',
          usage: 'Use upsert_code_block for structural changes or ast_edit for precise modifications',
          warnings: ['Ensure you understand the code structure before making changes']
        };
      } else {
        return {
          primaryTool: 'upsert_code_block',
          alternatives: ['ast_edit'],
          reasoning: 'Previous replace operation failed, avoiding text-based tools',
          usage: 'Use AST-based tools for more reliable editing after replace failure',
          warnings: ['Replace tool failed previously, avoid text-based matching']
        };
      }
    }
    ...
    const codeFiles = context.referencedFiles.filter(f => this.isCodeFile(f));
    const configFiles = context.referencedFiles.filter(f => this.isConfigFile(f));

    if (context.operationType === 'modify' && codeFiles.length > 0) {
      if (context.modificationType === 'function' || context.modificationType === 'class') {
        return {
          primaryTool: 'upsert_code_block',
          alternatives: ['ast_edit', 'replace'],
          reasoning: 'Structural code modification detected in code files',
          usage: 'Use upsert_code_block to replace/insert complete functions or classes',
      ...
      if (context.modificationType === 'variable') {
        return {
          ...
          usage: 'Use ast_edit with precise AST queries to modify variables or expressions',
          warnings: ['Ensure the AST query is specific enough to avoid unintended matches']
        };
      }
    }
    ...
    if (context.operationType === 'modify' && configFiles.length > 0) {
      return {
        ...
        warnings: ['Include sufficient context (5+ lines) to avoid ambiguous matches']
      };
    }
    ...
    return {
      ...
      alternatives: codeFiles.length > 0 ? ['ast_edit', 'replace'] : ['write_file'],
   ...
   */
  private static formatGuidance(context: TaskContext, recommendation: ToolRecommendation): string {
    const sections = [
    ...
    if (recommendation.warnings && recommendation.warnings.length > 0) {
      ...
      recommendation.warnings.forEach(warning => sections.push(`- ${warning}`));
      sections.push('');
    }
    ...
    if (codeFiles.length > 0) {
      sections.push(`- **Code files** (${codeFiles.join(', ')}): Prefer AST tools (upsert_code_block, ast_edit)`);
    }
    if (configFiles.length > 0) {
      sections.push(`- **Config files** (${configFiles.join(', ')}): Use replace with sufficient context`);
    }
    
    sections.push('');
    sections.push('**IMPORTANT**: This guidance is specific to the current task context. Follow it precisely.');
    sections.push('---');
    sections.push('');
  ...
  private static isCodeFile(filename: string): boolean {
    const codeExtensions = ['.ts', '.js', '.tsx', '.jsx', '.py', '.java', '.cpp', '.c', '.h', '.hpp', '.cs', '.php', '.rb', '.go', '.rs'];
  ...
  private static isConfigFile(filename: string): boolean {
    ...
    const configFiles = ['package.json', 'tsconfig.json', 'webpack.config.js', '.gitignore', 'Dockerfile'];
