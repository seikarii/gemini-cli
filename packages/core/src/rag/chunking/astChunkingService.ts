...
export class RAGASTChunkingService {
  ...
  async chunkSourceCode(
    ...
    try {
      ...
      chunks.push(
        ...this.extractClasses(sourceFile, filePath, language, metadata),
      );
      chunks.push(
        ...this.extractFunctions(sourceFile, filePath, language, metadata),
      );
      chunks.push(
        ...this.extractInterfaces(sourceFile, filePath, language, metadata),
      );
      chunks.push(
        ...this.extractTypes(sourceFile, filePath, language, metadata),
      );
      chunks.push(
        ...this.extractConstants(sourceFile, filePath, language, metadata),
      );
      chunks.push(
        ...this.extractImportExports(sourceFile, filePath, language, metadata),
      );

      // If no semantic chunks were found, fallback to text chunking
      if (chunks.length === 0) {
        this.logger.debug(
          `No semantic chunks found in ${filePath}, falling back to text chunking`,
        );
        return this.chunkAsText(content, filePath, language, metadata);
      }
      ...
      this.logger.debug(
        `Extracted ${chunks.length} semantic chunks from ${filePath}`,
      );
      return chunks;
    } catch (error) {
  ...
  private extractClasses(
    ...
    for (const classNode of classes) {
      ...
      for (const method of methods) {
        ...
        const { start: methodStart, end: methodEnd } =
          this.getNodeLineNumbers(method);
  ...
  private extractConstants(
    ...
    for (const varDecl of variableDeclarations) {
      ...
      const isConstant =
        varDecl.getParent()?.getParent()?.getKind() === SyntaxKind.ConstKeyword;
  ...
  private extractImportExports(
    ...
    if (imports.length > 0 || exports.length > 0) {
      ...
      const combinedText = [importsText, exportsText]
        ...
        chunks.push(
          this.createChunk({
            ...
            source: this.createFileSource(
              filePath,
              1,
              Math.max(imports.length, exports.length) + 1,
            ),
            language,
            metadata: {
              ...metadata,
              code: {
                ...
                exports: exports.map(
                  (exp) => exp.getModuleSpecifierValue() || 'default',
                ),
              },
            },
          }),
...
${sourceFile.getFullText().substring(0, 500)}...`;
  ...
  private chunkAsText(
    ...
    const overlapSize = Math.floor(
      maxChunkSize * this.config.chunking.overlapRatio,
    );
  ...
  private calculateComplexity(content: string): number {
    ...
    return Math.min(
      1,
      (nestingChars + specialChars + keywords * 2) / content.length,
    );
  }
  ...
  private calculateCompleteness(content: string): number {
    ...
    const braceBalance =
      openBraces === 0 ? 1 : Math.min(1, closeBraces / openBraces);
    const parenBalance =
      openParens === 0 ? 1 : Math.min(1, closeParens / openParens);
