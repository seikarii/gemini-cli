import { RAGVectorStore, RAGChunk, VectorStoreConfig, ScoredChunk, VectorStoreStats, QueryFilters, RAGVectorStoreError, ChunkType, ChunkSource, ChunkMetadata } from '../types.js';
...
export class RAGChromaVectorStore extends RAGVectorStore {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new RAGVectorStoreError(`ChromaVectorStore initialization failed: ${error}`);
    }
  }
  ...
  private async initializeWithRetry(): Promise<void> {
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        this.logger.warn(`Chroma connection attempt ${attempt}/${this.maxRetries} failed, retrying...`, { error });
  ...
  private async initializeClient(): Promise<void> {
    try {
      ...
      if (!chromaModule) {
        throw new Error('ChromaDB client not available. Install with: npm install chromadb');
      }
      ...
      throw new RAGVectorStoreError(`Failed to initialize Chroma client: ${error}`);
    }
  }
  ...
  private async loadChromaClient(): Promise<any> {
    try {
      ...
      this.logger.warn('ChromaDB not installed. Falling back to memory store for development.');
      return null;
    }
  }
  ...
  async addChunks(chunks: RAGChunk[]): Promise<void> {
    ...
    try {
      ...
      this.logger.debug('Added chunks to Chroma', { count: validChunks.length });
    } catch (error) {
  ...
  async deleteChunks(chunkIds: string[]): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to delete chunks from ChromaVectorStore', { error, chunkIds });
      throw error;
    }
  }
  ...
  async getChunk(id: string): Promise<RAGChunk | null> {
    ...
    try {
      ...
      if (results.ids && results.ids.length > 0 && results.documents) {
        ...
        const embedding = results.embeddings ? results.embeddings[0] : undefined;
      ...
      this.logger.error('Failed to get chunk from ChromaVectorStore', { error, id });
      throw error;
    }
  }
